...
if ! az network vnet show --resource-group "$RESOURCE_GROUP" --name "$VNET_NAME" --only-show-errors &>/dev/null; then
    ...
    echo ""
    log_warn "Please run ./00-1-create-network.sh first to create the network infrastructure"
    exit 1
fi
...
log_info "  • Service CIDR: $SERVICE_CIDR (for Kubernetes services)"
log_info "  • DNS Service IP: $DNS_SERVICE_IP"
